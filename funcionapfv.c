...
/// PREENCHE O MAPA AUXILIAR =============================================
    ...
    if(var==1)
    {
        ...
        bin = malloc ( (bytes*8) * sizeof (char *));
        ...
        aux = k+(9*k)-1;//Do (30+9*30)-1 ao 30
        ...
        aux = tamanhoBytarada-1; //do ultimo ao (30+9*30)
        ...
        for(i=0;i<bytes; i++)
        {
            scrr = malloc ( (8) * sizeof (char *));
            for(j=cont; j<(cont+8); j++)
            {
                scrr[j-cont]=(bin[j]);
                //printf("%d",(bin[j]));
            ...
            cont+=8;
            strncpy((char *)p+i, scrr, 8);
            for(j=0; j<8; j++)
            {
                printf("%d",((p+i)[j]&(1<<j))>0);
...
/// SEPARA NAS MATRIZES on/off E coordenadas (x, y) =============================================
    ...
    //ext++;
    //printf("\n %d) ERRO = %f - SAIDA = %f - PERCENT = %f ",ext,err,1./(err),porcentagem);
    //wait_for_char();
