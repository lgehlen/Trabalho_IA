...
int tamanhoBuffer, linha = 0, coluna = 0, bytes=75, i, j, cont, cont2, var=0, km, ext=0;

float diagonal, porcentagem, contaParana=0., contaPreenchido=0.; //((k*9)+(k*10)+k)/8
...
void main()
{
    int contaColuna = 0, contaLinha = 0, coordenadasRand[k][2], xRand, yRand, aux=0;
    ...
    for(i=0;i<k;i++)
    {
        ...
        for(j=cont2;j<cont2+9;j++)
        {
            if(stringBits[j] == '1')
            {
                ...
                {
                    coordenadas[i][0] = coordenadas[i][0] * 2 + 1;
                }
            }
            else
            {
            ...
            //xF = (coordenadas[i][0] * 0.97);
            if(coordenadas[i][0]>linha)
            {
                coordenadas[i][0] = round((double) coordenadas[i][0] * 0.97);
            }
            //coordenadas[i][0] = (int) xF;
            cont++;
        }
        cont2 = cont;
    }
    ...
    for(i=0;i<k;i++)
    {
        ...
        for(j=cont2;j<cont2+10;j++)
        {
            if(stringBits[j] == '1')
            {
                ...
                {
                    coordenadas[i][1] = coordenadas[i][1] * 2 + 1;
                }
            }
            else
            {
            ...
            //yF = (coordenadas[i][1] * 0.71);
            if(coordenadas[i][1]>coluna)
            {
                coordenadas[i][1] = round((double) coordenadas[i][1] * 0.71);
            }
            //coordenadas[i][1] = (int) yF;
            cont++;
        }
        cont2 = cont;
    }
...
float func1(unsigned char *p, int n)
{
    ...
    int i, j, l, xRand, yRand, xRandRaio, yRandRaio, aux = 0, cima, esquerda, retoX, retoY, posX, posY, posX2, posY2, cont3;

    float contaNPreenchido, *vcontador, err=0;

    char x1;

    cont = 0;
    contaNPreenchido = contaParana;
